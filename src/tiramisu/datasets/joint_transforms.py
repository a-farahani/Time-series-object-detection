...
class JointRandomSizedCrop(object):
    ...
    def __call__(self, imgs):
        area_lo, area_hi = 0.45, 1.0
        ratio_lo, ratio_hi = 0.5, 2.0
        for attempt in range(10):
            area = imgs[0].size[0] * imgs[0].size[1]
            target_area = random.uniform(area_lo, area_hi) * area
            aspect_ratio = random.uniform(ratio_lo, ratio_hi)
            ...
            if w <= imgs[0].size[0] and h <= imgs[0].size[1]:
                ...
                return [imgs[0].resize((self.size, self.size), self.interpolation), imgs[1].resize((self.size, self.size), self.mask_interpolation)]
